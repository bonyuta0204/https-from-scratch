...
impl HttpClient for Http1Client {
    fn request(&self, req: Request) -> Result<Response, Error> {
        let mut stream = TcpStream::connect("www.google.com:80").map_err(|_| Error::ConnectionFailed)?;
        ...
        // Send request
        stream.write_all(request_line.as_bytes()).map_err(|e| Error::Other(e.to_string()))?;
        stream.write_all(headers.as_bytes()).map_err(|e| Error::Other(e.to_string()))?;
        stream.write_all(b"\r\n").map_err(|e| Error::Other(e.to_string()))?;
        stream.write_all(&body).map_err(|e| Error::Other(e.to_string()))?;

        // Read response
        let mut buffer = [0; 4096];
        let bytes_read = stream.read(&mut buffer).map_err(|e| Error::Other(e.to_string()))?;
