...
/// A simple structure to hold an HTTP/2 frame.
struct Frame {
    length: u32,
    frame_type: u8,
    flags: u8,
...
fn main() -> std::io::Result<()> {
    ...
    let mut config = ClientConfig::builder()
        ...
        .with_no_client_auth();
    // Add the well-known roots from webpki-roots.
    config
        .root_store
        .add_server_trust_anchors(TLS_SERVER_ROOTS.0.iter().map(|ta| {
    ...
    println!("Sending connection preface:\n{}", String::from_utf8_lossy(preface));
    ...
    tls.write_all(&client_settings.encode())?;
    tls.flush()?;
    ...
    loop {
        ...
        if frame.frame_type == 0x4 {
            ...
            tls.write_all(&settings_ack.encode())?;
            tls.flush()?;
            break;
        }
    }
    ...
    println!("Constructed HPACK header block ({} bytes):", header_block.len());
    print!("  ");
    ...
    tls.write_all(&headers_frame.encode())?;
    tls.flush()?;
